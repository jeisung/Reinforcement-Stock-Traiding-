...
class DQN:
    ...
    def __init__(self, session, day_length, data_kind, n_action):
        self.session = session
        self.n_action = n_action
        self.width = day_length
        self.height = day_length
        self.STATE_LEN = data_kind
        # 게임 플레이결과를 저장할 메모리
        self.memory = deque()
        # 현재 게임판의 상태
        self.state = None
        ...
        self.input_X = tf.placeholder(tf.float32, [None, self.width, self.height, self.STATE_LEN])
    ...
    def _build_network(self, name):
        with tf.variable_scope(name):
            ...
            model = tf.layers.max_pooling2d(model, pool_size=[2,2], strides=2, padding="SAME" )
            model = tf.layers.conv2d(model, 32, [3, 3], padding='same', activation=tf.nn.relu)
            model = tf.layers.max_pooling2d(model, pool_size=[2,2], strides=2, padding="SAME" )
    ...
    def init_state(self, state):
        ...
        state = np.reshape(state, (self.width, 1, -1))
        #state = [state for _ in range(self.width)]
    ...
    def remember(self, state, action, reward, terminal):
        ...
        # 이 모델에서는 과거 19번 + 현재 = 총 20번의 상태를 계산하도록 하였으며,
        # 새로운 상태가 들어왔을 때, 가장 오래된 상태를 제거하고 새로운 상태를 넣습니다.
        next_state = np.reshape(state, (self.width, 1, -1))
